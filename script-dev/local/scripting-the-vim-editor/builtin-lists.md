...
- [Sublists](#N10227)
- [Example 1:
                Revisiting autoalignments](#N10241)
- [Example 2:
                Enhancing Vim’s completion facilities](#N10321)
- [Looking
                ahead](#N105BD)
...
Scripting the Vim editor, Part 3
...
##### Listing 7.
                    Finding size, range, and indexes
...
##### Listing 8.
                    Generating a list using the range() function
...
##### Listing 9.
                    Generating a list by splitting text
...
You can explore the many other list-related functions by typing `:help function-list` in any Vim session, then
...
##### Listing 12.
                    Adding a set of values to a list
...
Doing so would almost always be a serious mistake, however, because even
                ...
                sorted-and-reversed list (as described under "[List
                    assignments and aliasing](#assignments)").

This is highly counterintuitive for most programmers, who typically expect
...
You can concatenate lists with the *+* and *+=* operators, like
            so:
...
If you omit the starting index, the sublist automatically starts at zero;
...
## Example 2:
                Enhancing Vim’s completion facilities
...
##### Listing 21.
                    Text completion with alternatives
...
leaving the cursor conveniently in the middle of the newly closed
                block.
...
The function also needs to know the text on the current line, which can be
                retrieved using `getline()`, and is stored in
                    `curr_line`.
...
##### Listing 26.
                    A cleaner shorthand for iterating over nested lists
...
##### Listing 27.
                    Restoring the cursor after a one-character insertion at
                    end-of-line
...
##### Listing 28.
                    Falling back to CTRL-N behavior
...
##### Listing 29.
                    Falling back to normal TAB key behavior
...
The key-mapping converts any insert-mode **TAB** to a **CTRL-R=**,
                ...
                completion string it returns (see `:help
                    i_CTRL-R` or the [first article in this
                    series](http://www.ibm.com/developerworks/linux/library/l-vim-script-1/index.html) for details of this mechanism).

The `inoremap` form of `imap` is used here because some of the completion strings that
                    `SmartComplete()` returns also contain a
                    **TAB** character. If a regular `imap`
...
With the `inoremap` in place, we now have a
                    **TAB** key that can:
...
In addition, with the code from Listings 22 and 23 placed in your .vimrc
                ...
                extending the completion table with extra calls to `AddCompletion()`. For example, you could make it easier to
                start new Vimscript functions with:

    call AddCompletion( 'function!\?',  "",  "\<CR>endfunction", 1 )

so that tabbing immediately after a function keyword appends the
                corresponding `endfunction` keyword on the next
                line.
...
- "[Scripting the Vim editor, Part 2: User-defined functions](http://www.ibm.com/developerworks/linux/library/l-vim-script-2/index.html)"
                (developerWorks, July 2009) tours Vimscript’s scalar data types: 
